...
function detectEnvironment(text) {
    if (/(本地|local|localhost)/i.test(text))
        return 'local';
    if (/(预览|preview)/i.test(text))
        return 'preview';
...
function isDeploymentEnvironmentCandidate(value) {
    return DEPLOYMENT_ENVIRONMENT_TEXT.test(value);
}
function sanitizeTargetCandidate(value) {
    const candidate = String(value || '').trim().replace(/^[\s,，。！？!?;；:：]+|[\s,，。！？!?;；:：]+$/g, '');
    if (!candidate)
        return '待确认目标';
    if (isDeploymentEnvironmentCandidate(candidate))
        return '待确认目标';
    if (DEPLOYMENT_ACKNOWLEDGEMENTS.has(candidate.toLowerCase()))
        return '待确认目标';
    if (DEPLOYMENT_FILLER_TOKENS.has(candidate.toLowerCase()))
        return '待确认目标';
    return candidate;
}
function isLikelyLooseTarget(text) {
    const trimmed = String(text || '').trim();
    if (!trimmed || trimmed.length > 20)
        return false;
    if (/\s/.test(trimmed))
        return false;
    if (DEPLOYMENT_ACKNOWLEDGEMENTS.has(trimmed.toLowerCase()))
        return false;
    if (DEPLOYMENT_FILLER_TOKENS.has(trimmed.toLowerCase()))
        return false;
    if (isDeploymentEnvironmentCandidate(trimmed))
        return false;
...
function detectTarget(text, allowLoose = false) {
    ...
    if (english?.[1])
        return sanitizeTargetCandidate(english[1]);
    ...
    if (allowLoose) {
        ...
        if (isLikelyLooseTarget(text))
            return sanitizeTargetCandidate(text);
    }
    return '待确认目标';
}
function missingFieldsFor(target, environment) {
    const missingFields = [];
    if (target === '待确认目标')

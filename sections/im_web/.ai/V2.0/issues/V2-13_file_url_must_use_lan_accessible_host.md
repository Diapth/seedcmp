...
2026-05-24 回归发现：点击文件消息时，前端传给 `window.open()` 的 URL 已经是 `http://100.79.157.76:8090/v1/file/preview/...`，但后端预览接口会重定向到 MinIO 直链 `http://127.0.0.1:9000/...`。浏览器会跟随该 Location，导致局域网设备仍然无法下载文件。
...
| Browser Audit | 文件卡片点击 URL 拦截审计 | Pass，打开 `http://100.79.157.76:9000/...`，不含 `127.0.0.1` |
...
8. `/file/preview/...` 链接直接映射为对象存储直链 `http://100.79.157.76:9000/...`，并支持 `VITE_OBJECT_STORAGE_BASE_URL` / `VITE_MINIO_BASE_URL` 覆盖。
...
| 文件卡片点击 URL 拦截审计 | Pass，`window.open()` 入参为 `http://100.79.157.76:9000/...` |

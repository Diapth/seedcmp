...
export const useConversationStore = defineStore('conversation', () => {
    ...
    const manuallyDeletedConversationKeys = ref({});
    const draftSyncTimers = ref({});
    const channelStore = useChannelStore();
    const userStore = useUserStore();
    function getConversationKey(channelId, channelType) {
        return `${String(channelId)}-${Number(channelType)}`;
    }
    function findConversation(channelId, channelType) {
    ...
    function isConversationDigestSource(raw) {
        return raw && ![99, 1000].includes(getMessageType(raw));
    }
    function getLastMessageSource(item) {
    ...
    function normalizeConversationInput(item, info) {
    ...
    function upsertConversation(next) {
        ...
        if (manuallyDeletedConversationKeys.value[key])
            return;
    ...
    async function ensureConversation(channelId, channelType, message) {
        channelId = String(channelId);
        channelType = Number(channelType);
        const key = getConversationKey(channelId, channelType);
        if (manuallyDeletedConversationKeys.value[key])

...
func (rb *Robot) messagesListen(messages []*config.MessageResp) {
	for _, message := range messages {
		...
		if contentMap != nil && contentMap["robot_id"] != nil {
			...
			if robotID != nil {
				if robotID == rb.ctx.GetConfig().Account.SystemUID {
					...
					if entities != nil {
						...
						for _, entitiesObj := range entities {
							entitiesMap, _ := entitiesObj.(map[string]interface{})
							if entitiesMap == nil {
								continue
							}
							if entitiesMap["type"] == "bot_command" {
								offset = jsonNumberToInt64(entitiesMap["offset"])
								length = jsonNumberToInt64(entitiesMap["length"])
								break
							}
						}
						...
						if start >= 0 && end <= len(contentRuns) && start < end {
							key = string(contentRuns[start:end])
						}
					}

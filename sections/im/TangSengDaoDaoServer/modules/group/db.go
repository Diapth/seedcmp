...
func (d *DB) querySavedGroups(uid string) ([]*DetailModel, error) {
	var detailModels []*DetailModel
	_, err := d.session.Select("`group`.*,IFNULL(group_setting.version,0) + `group`.version  version,IFNULL(group_setting.chat_pwd_on,0) chat_pwd_on,IFNULL(group_setting.mute,0) mute,IFNULL(group_setting.top,0) top,IFNULL(group_setting.show_nick,0) show_nick,IFNULL(group_setting.save,1) save,IFNULL(group_setting.remark,'') remark").From("group_member").LeftJoin("group", "`group`.group_no=group_member.group_no").LeftJoin("group_setting", "`group`.group_no=group_setting.group_no and group_setting.uid=group_member.uid").Where("group_member.uid=? and group_member.is_deleted=0 and group_member.status=1 and `group`.status=?", uid, GroupStatusNormal).Load(&detailModels)
	return detailModels, err
}

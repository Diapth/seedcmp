...
func (d *DB) QueryByUsername(username string) (*Model, error) {
	var model *Model
	query := "username=? or concat(zone,phone)=? or email=?"
	args := []interface{}{username, username, username}
	// Web UI 登录时会加"00"前缀(如"0086"+"手机号")，同时也尝试匹配去掉"00"前缀
	if strings.HasPrefix(username, "00") && len(username) > 2 {
		stripped := username[2:]
		query += " or username=? or concat(zone,phone)=?"
		args = append(args, stripped, stripped)
	}
	_, err := d.session.Select("*").From("user").Where(query, args...).Load(&model)
	return model, err
}

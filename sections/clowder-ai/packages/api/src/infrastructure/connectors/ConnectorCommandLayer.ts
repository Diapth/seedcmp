...
function normalizeImWebCatClientId(value: string): ClientId | undefined {
...
function parseImWebCatClientId(parts: string[]): ClientId | undefined {
  for (let idx = 0; idx < parts.length; idx += 1) {
    const part = parts[idx]?.trim();
    if (!part) continue;
    const lower = part.toLowerCase();
    if (lower === '--platform' || lower === '--client' || lower === '--provider') {
      return normalizeImWebCatClientId(parts[idx + 1] ?? '');
    }
    const inline = lower.match(/^--(?:platform|client|provider)=(.+)$/);
    if (inline) return normalizeImWebCatClientId(inline[1] ?? '');
  }
  return undefined;
}

function parseFlagValue(parts: string[], names: string[]): string | undefined {
  const wanted = new Set(names.map((name) => name.toLowerCase()));
  for (let idx = 0; idx < parts.length; idx += 1) {
    const part = parts[idx]?.trim();
    if (!part) continue;
    const lower = part.toLowerCase();
    if (wanted.has(lower)) return parts[idx + 1]?.trim();
    for (const name of wanted) {
      const inline = lower.match(new RegExp(`^${name.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}=(.+)$`));
...
export interface ConnectorCommandLayerDeps {
  ...
  /** Create a runtime cat from connector command surfaces. */
  readonly catCreator?: {
    create(input: {
      displayName: string;
      ...
      defaultModel?: string;
      requestedBy: string;
    }): Promise<{ catId: string; displayName: string; mentionPatterns: string[] }>;
  };
...
export class ConnectorCommandLayer {
  ...
  private async dispatch(
    ...
    switch (cmd) {
      ...
      case '/cats':
        return this.handleCats(connectorId, externalChatId, cmdArgs, senderId ?? userId);
      case '/status':
  ...
  private async handleCats(
    ...
    if (trimmed.toLowerCase().startsWith('new')) {
      return this.handleCatsNew(binding.threadId, trimmed.slice(3).trim(), requestedBy);
    }
    return buildCatsInfo(binding.threadId, this.deps);
  }

  private async handleCatsNew(threadId: string, args: string, requestedBy: string): Promise<CommandResult> {
    if (!args) {
      return {
        kind: 'cats',
    ...
    if (!this.deps.catCreator) {
      return { kind: 'cats', response: '⚠️ 新增猫猫能力未启用。', contextThreadId: threadId };
    }
    ...
    const clientId = parseImWebCatClientId(parts);
    if (!clientId) {
      return {
        kind: 'cats',
    ...
    const explicitAliases = parts.slice(1).filter((part) => part.startsWith('@'));
    const mentionPatterns = explicitAliases.length > 0 ? explicitAliases : [`@${displayName}`];

    try {
      const cat = await this.deps.catCreator.create({
        displayName,
        mentionPatterns,
        clientId,
        authType,
      ...
      if (this.deps.catRoster) {
        this.deps.catRoster[cat.catId] = { displayName: cat.displayName, available: true };
      }
      return {
        kind: 'cats',
        response: `✅ 已新增猫猫：${cat.displayName}\nID: ${cat.catId}\n别名：${cat.mentionPatterns.join(', ')}\n\n现在可以用 ${cat.mentionPatterns[0]} 或 /ask ${cat.catId} 发起对话。`,
        contextThreadId: threadId,
      };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      return { kind: 'cats', response: `❌ 新增猫猫失败：${message}`, contextThreadId: threadId };
    }
  }

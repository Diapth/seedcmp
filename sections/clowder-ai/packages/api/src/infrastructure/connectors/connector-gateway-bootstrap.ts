...
import { ImWebAdapter } from './adapters/ImWebAdapter.js';
...
import { ImWebInboundHandler } from './ImWebInboundHandler.js';
...
export interface ConnectorGatewayConfig {
  ...
  imWebEnabled?: boolean | undefined;
  imWebConnectorSecret?: string | undefined;
  imWebOutboundCallbackUrl?: string | undefined;
  imWebRequestTimeoutMs?: number | undefined;
}
...
export interface ConnectorGatewayDeps {
  ...
  /** IM Web connector can create runtime cats through /cats new. */
  readonly catCreator?: ConnectorCommandLayerDeps['catCreator'];
...
export function loadConnectorGatewayConfig(): ConnectorGatewayConfig {
  return {
    ...
    imWebEnabled: process.env.IM_WEB_CLOWDER_ENABLED === 'true',
    imWebConnectorSecret: process.env.CLOWDER_CONNECTOR_SECRET,
    imWebOutboundCallbackUrl: process.env.CLOWDER_OUTBOUND_CALLBACK_URL,
    imWebRequestTimeoutMs: process.env.CLOWDER_REQUEST_TIMEOUT_MS
      ? Number(process.env.CLOWDER_REQUEST_TIMEOUT_MS)
      : undefined,
  };
}
...
export async function startConnectorGateway(
  ...
  const hasImWeb = Boolean(config.imWebEnabled);
  ...
  const commandLayer = new ConnectorCommandLayer({
    ...
    catRoster,
    ...(deps.catCreator ? { catCreator: deps.catCreator } : {}),
  ...
  if (hasImWeb) {
    const imWeb = new ImWebAdapter(log, {
      outboundCallbackUrl: config.imWebOutboundCallbackUrl,
      connectorSecret: config.imWebConnectorSecret,
      requestTimeoutMs: config.imWebRequestTimeoutMs,
    });
    adapters.set('im-web', imWeb);
    webhookHandlers.set(
      'im-web',
      new ImWebInboundHandler({
        connectorSecret: config.imWebConnectorSecret,
        router: connectorRouter,
        signatureToleranceMs: process.env.CLOWDER_SIGNATURE_TOLERANCE_MS
          ? Number(process.env.CLOWDER_SIGNATURE_TOLERANCE_MS)
          : undefined,
        log,
      }),
    );
    log.info(
      { configured: imWeb.isConfigured() },
      '[ConnectorGateway] IM Web adapter and inbound handler registered (TangSeng bridge mode)',
    );
  }

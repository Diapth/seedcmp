...
describe('/cats new live registry behavior', { concurrency: false }, () => {
  ...
  it('honors explicit stable cat ids from IM Web template creation', async () => {
    await withTempProject(async () => {
      const created = await createImWebCatCreator().create({
        catId: 'riverwatch-source-curator-934553',
        displayName: '狸花猫（资料整理师）',
        mentionPatterns: ['@source-curator'],
      ...
      assert.equal(created.catId, 'riverwatch-source-curator-934553');
      assert.equal(catRegistry.has('riverwatch-source-curator-934553'), true);
      assert.equal(catRegistry.has('runtime-cat-source-curator'), false);
      assert.equal(catRegistry.tryGet(created.catId)?.config.accountRef, 'claude');
    });
  });

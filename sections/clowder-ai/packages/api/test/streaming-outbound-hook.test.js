...
describe('StreamingOutboundHook', () => {
  ...
  it('keeps runtime coordinator-style PM placeholders in the IM Web PM direct binding', async () => {
    ...
    const bindingStore = createBindingStore([
      {
        connectorId: 'im-web',
        externalChatId: `1:user-1@clowder_cat:${runtimePmCatId}`,
        threadId: 'thread-project',
        userId: 'u1',
        createdAt: Date.now(),
      },
    ...
    await hook.onStreamStart('thread-project', runtimePmCatId);

    assert.equal(adapter._calls.sendPlaceholder.length, 1);
    assert.equal(adapter._calls.sendPlaceholder[0].chatId, `1:user-1@clowder_cat:${runtimePmCatId}`);
  });
  ...
  it('onStreamChunk rate limits retry storms after editMessage failures', async () => {
    const adapter = wrapAdapter({
      connectorId: 'telegram',
      sendReply: async () => {},
      sendPlaceholder: async () => 'msg-placeholder-1',
      editMessage: async () => {
    ...
    const bindingStore = createBindingStore([
      { connectorId: 'telegram', externalChatId: 'tg-chat', threadId: 'thread-1', userId: 'u1', createdAt: Date.now() },
    ]);
    const log = {
      warn: () => {},
      info: () => {},
    ...
    const hook = new StreamingOutboundHook({ bindingStore, adapters, log, updateIntervalMs: 5, minDeltaChars: 0 });

    await hook.onStreamStart('thread-1');
    await new Promise((resolve) => setTimeout(resolve, 10));
    await hook.onStreamChunk('thread-1', 'chunk after failure');
    await hook.onStreamChunk('thread-1', 'chunk after failure plus more text');

    assert.equal(adapter._calls.editMessage.length, 1);
  });

  it('onStreamChunk does not write token-stream edits into IM Web durable chat', async () => {
    const adapter = wrapAdapter({
      connectorId: 'im-web',
      sendReply: async () => {},
      sendPlaceholder: async () => 'msg-placeholder-1',
    ...
    const adapters = new Map([['im-web', adapter]]);
    const bindingStore = createBindingStore([
      { connectorId: 'im-web', externalChatId: '1:clowder_ai', threadId: 'thread-1', userId: 'u1', createdAt: Date.now() },
    ]);
    const log = {
      warn: () => {},
      info: () => {},
    ...
    await hook.onStreamStart('thread-1');
    await hook.onStreamChunk('thread-1', 'partial output that should stay out of TangSeng history');

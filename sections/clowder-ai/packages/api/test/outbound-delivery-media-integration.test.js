...
describe('OutboundDeliveryHook — media delivery integration', () => {
  ...
  it('publishes file:// media_gallery images before sending native media', async () => {
    const { OutboundDeliveryHook } = await import('../dist/infrastructure/connectors/OutboundDeliveryHook.js');

    const tempDir = await mkdtemp(join(tmpdir(), 'cat-cafe-file-url-image-'));
    const uploadDir = await mkdtemp(join(tmpdir(), 'cat-cafe-file-url-upload-'));
    const previousUploadDir = process.env.UPLOAD_DIR;
    process.env.UPLOAD_DIR = uploadDir;
    try {
      const imagePath = join(tempDir, 'sinx_plot.png');
      ...
      const hook = new OutboundDeliveryHook({
        bindingStore: {
          async getByThread() {
            return [{ connectorId: 'im-web', externalChatId: 'chat1', threadId: 'T1', userId: 'u1', createdAt: 0 }];
          },
        },
        adapters: new Map([
          [
      ...
      await hook.deliver('T1', 'generated sin plot', undefined, [
        {
          id: 'sinx-plot-001',
          kind: 'media_gallery',
          v: 1,
          title: 'y = sin(x)',
          items: [{ url: pathToFileURL(imagePath).href, caption: 'y = sin(x)' }],
        },
      ]);
      ...
      assert.equal(sendMediaCalls[0].chatId, 'chat1');
      assert.equal(sendMediaCalls[0].payload.type, 'image');
      assert.match(sendMediaCalls[0].payload.url, /^http:\/\/localhost:3003\/uploads\/sinx-plot-001-/);
      assert.match(sendMediaCalls[0].payload.absPath, /sinx-plot-001-.*\.png$/);
      assert.equal(sendMediaCalls[0].payload.alt, 'y = sin(x)');
    } finally {
  ...
  it('publishes absolute-path media_gallery images before sending native media', async () => {
    const { OutboundDeliveryHook } = await import('../dist/infrastructure/connectors/OutboundDeliveryHook.js');

    const tempDir = await mkdtemp(join(tmpdir(), 'cat-cafe-absolute-image-'));
    const uploadDir = await mkdtemp(join(tmpdir(), 'cat-cafe-absolute-upload-'));
    const previousUploadDir = process.env.UPLOAD_DIR;
    process.env.UPLOAD_DIR = uploadDir;
    try {
      const imagePath = join(tempDir, 'sinx_plot.png');
      ...
      await hook.deliver('T1', 'generated sin plot', undefined, [
        {
          id: 'sinx-plot-absolute',
          kind: 'media_gallery',
          v: 1,
          title: 'y = sin(x)',
          items: [{ url: imagePath, caption: 'y = sin(x)' }],
        },
      ]);

      assert.equal(sendMediaCalls.length, 1);
      assert.equal(sendMediaCalls[0].payload.type, 'image');
      assert.match(sendMediaCalls[0].payload.url, /^http:\/\/localhost:3003\/uploads\/sinx-plot-absolute-/);
      assert.match(sendMediaCalls[0].payload.absPath, /sinx-plot-absolute-.*\.png$/);
    } finally {
      if (previousUploadDir === undefined) delete process.env.UPLOAD_DIR;
      else process.env.UPLOAD_DIR = previousUploadDir;
      await rm(tempDir, { recursive: true, force: true });
  ...
  it('publishes local image paths mentioned in final text before sending native media', async () => {
    const { OutboundDeliveryHook } = await import('../dist/infrastructure/connectors/OutboundDeliveryHook.js');

    const tempDir = await mkdtemp(join(tmpdir(), 'cat-cafe-text-image-'));
    const uploadDir = await mkdtemp(join(tmpdir(), 'cat-cafe-text-upload-'));
    const previousUploadDir = process.env.UPLOAD_DIR;
    process.env.UPLOAD_DIR = uploadDir;
    try {
      const imagePath = join(tempDir, 'sinx_plot.png');
      ...
      await hook.deliver('T1', `PNG 图片已保存至 ${imagePath}，请通过聊天框发送。`);

      assert.equal(sendMediaCalls.length, 1);
      assert.equal(sendMediaCalls[0].chatId, 'chat1');
      assert.equal(sendMediaCalls[0].payload.type, 'image');
      assert.match(sendMediaCalls[0].payload.url, /^http:\/\/localhost:3003\/uploads\/text-image-T1-/);
      assert.match(sendMediaCalls[0].payload.absPath, /text-image-T1-.*\.png$/);
    } finally {
      if (previousUploadDir === undefined) delete process.env.UPLOAD_DIR;
      else process.env.UPLOAD_DIR = previousUploadDir;
      await rm(tempDir, { recursive: true, force: true });
  ...
  it('publishes data URI media_gallery images before sending IM Web native media', async () => {
    const { OutboundDeliveryHook } = await import('../dist/infrastructure/connectors/OutboundDeliveryHook.js');

    const uploadDir = await mkdtemp(join(tmpdir(), 'cat-cafe-data-uri-upload-'));
    const previousUploadDir = process.env.UPLOAD_DIR;
    process.env.UPLOAD_DIR = uploadDir;
    try {
      const sendMediaCalls = [];
      ...
      await hook.deliver('T1', 'generated sin plot', undefined, [
        {
          ...
          v: 1,
          title: 'y = sin(x)',
          items: [{ url: `data:image/png;base64,${Buffer.from('fake-png').toString('base64')}` }],
        },
      ]);

      assert.equal(sendMediaCalls.length, 1);
      assert.equal(sendMediaCalls[0].payload.type, 'image');
      assert.match(sendMediaCalls[0].payload.url, /^http:\/\/localhost:3003\/uploads\/sinx-data-uri-/);
      assert.match(sendMediaCalls[0].payload.absPath, /sinx-data-uri-.*\.png$/);
    } finally {
      if (previousUploadDir === undefined) delete process.env.UPLOAD_DIR;
      else process.env.UPLOAD_DIR = previousUploadDir;
      await rm(uploadDir, { recursive: true, force: true });

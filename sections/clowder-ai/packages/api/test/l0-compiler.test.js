...
import { filterAvailableTeammates } from '../../../scripts/compile-system-prompt-l0.mjs';
import {
...
test('compile L0 teammate roster hides old Codex runtime cats for Claude OAuth cats', () => {
  const allConfigs = {
    'riverpm-claude': {
      clientId: 'anthropic',
      accountRef: 'claude',
    ...
    'runtime-cat-old-codex': {
      clientId: 'openai',
      accountRef: 'codex',
      displayName: 'Old Codex Worker',
      mentionPatterns: ['@runtime-cat-old-codex'],
    },
    'runtime-cat-claude-source': {
      clientId: 'anthropic',
      accountRef: 'claude',
  ...
  const teammates = filterAvailableTeammates(allConfigs, 'riverpm-claude', () => true);
  const teammateIds = teammates.map(([id]) => id);

  assert.deepEqual(teammateIds, ['runtime-cat-claude-source']);
});

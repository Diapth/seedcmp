...
describe('F088 Phase B+4 Integration', () => {
  ...
  describe('streaming: placeholder → chunks → final', () => {
    ...
    it('keeps im-web token-stream chunks out of durable chat', async () => {
      ...
      assert.equal(imWebAdapter.sent.length, 1);
      assert.equal(imWebAdapter.sent[0].type, 'placeholder');
      assert.equal(imWebAdapter.edits.length, 0);
    });
  });

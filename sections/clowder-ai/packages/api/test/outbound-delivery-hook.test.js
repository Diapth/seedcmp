...
describe('OutboundDeliveryHook', () => {
  ...
  it('prefers IM Web project group binding over PM direct binding on the same thread', async () => {
    ...
    await hook.deliver('thread-project', 'worker output', 'codex');
  ...
  describe('file block delivery via sendMedia', () => {
    ...
    it('delivers unresolved IM Web workspace raw file URLs as native file cards', async () => {
      const mediaSent = [];
      const mediaAdapter = {
        connectorId: 'im-web',
        async sendReply() {},
      ...
      const blocks = [{
        ...
        kind: 'file',
        v: 1,
        url: '/api/workspace/file/raw?worktreeId=thread-riverwatch&path=source-pack.md',
        fileName: 'source-pack.md',
      }];
      await hook.deliver('thread-abc', 'Real workspace file attached', 'codex', blocks);

      assert.equal(mediaSent.length, 1);
      assert.equal(mediaSent[0].chatId, '2:group-clowder');
      assert.equal(mediaSent[0].payload.type, 'file');
      assert.equal(
        mediaSent[0].payload.url,
        'http://localhost:3004/api/workspace/file/raw?worktreeId=thread-riverwatch&path=source-pack.md',
      );
      assert.equal(mediaSent[0].payload.fileName, 'source-pack.md');
      assert.equal('absPath' in mediaSent[0].payload, false);
    });

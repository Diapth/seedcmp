...
describe('ConnectorCommandLayer', () => {
  ...
  describe('/cats (F142)', () => {
    ...
    it('/cats new creates a runtime cat through the injected creator', async () => {
      const created = [];
      const layer = new ConnectorCommandLayer({
        bindingStore: stubStore({
          connectorId: 'im-web',
          externalChatId: '2:group1',
          threadId: 't-bound',
          userId: 'user1',
        }),
        threadStore: stubThreadStore({ id: 't-bound', title: '测试' }),
      ...
      assert.ok(result.response.includes('wujing'));
      assert.ok(result.response.includes('@悟净'));
      assert.equal(result.contextThreadId, 't-bound');
    });

    it('/cats new reports unavailable creation support when catCreator is missing', async () => {
      const layer = new ConnectorCommandLayer({
        bindingStore: stubStore({
          connectorId: 'im-web',
          externalChatId: '2:group1',
      ...
      const result = await layer.handle('im-web', '2:group1', 'user1', '/cats new 悟净 @悟净 --platform codex');
      assert.equal(result.kind, 'cats');
      assert.ok(result.response.includes('新增猫猫能力未启用'));
      assert.equal(result.contextThreadId, 't-bound');
    });
  });

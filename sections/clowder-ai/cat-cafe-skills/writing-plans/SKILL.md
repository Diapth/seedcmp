...
**Context:** Write the plan before implementation. After the plan is committed, continue to `tdd` in the current branch or an isolated worktree if needed.
...
计划写完并提交 → `tdd`（开始实现）。需要隔离时先创建独立 branch/worktree。

      ...
      <view class="content-box flex-column">
        ...
        <view class="result-section flex-column" v-if="searchResult">
          ...
          <view class="action-panel flex-column">
            ...
            <view class="status-tip flex-row align-center gap-2" v-else-if="searchResult.relationship === 'pending_sent'">
              <AppIcon name="info" :size="16" color="var(--color-warning)" />
              <text class="tip-text warning">好友申请已发送，等待对方通过</text>
            </view>
...
async function handleSearch() {
  ...
  try {
    searchResult.value = await contactStore.searchNativeUser(query, {
      currentUid: appStore.currentUser?.id
    });
  } catch (error) {
    if (query.length < 2) {
    ...
    uni.showToast({ title: contactStore.nativeError || error?.msg || '已使用本地示例结果', icon: 'none' });
  }
}
...
async function sendRequest() {
  ...
  try {
    if (searchResult.value.nativeFallback) {
      contactStore.sendFriendRequest(searchResult.value.nickname, verificationMsg.value);
    } else {
      await contactStore.sendNativeFriendRequest(searchResult.value, verificationMsg.value);
    }
    searchResult.value.relationship = 'pending_sent';
    ...
    uni.showToast({ title: error?.msg || error?.message || '发送失败', icon: 'none' });
  } finally {
    sending.value = false;
  }
}

...
const memberMenuItems = computed(() => {
  ...
  const isSelf = selectedMember.value.id === 'me';
...
onMounted(async () => {
  ...
  const persistedKey = storage.get('active_conversation_key');
  const persistedId = storage.get('active_conversation_id');
  if (persistedKey || persistedId) {
    convStore.setActiveId(persistedKey || persistedId);
  }
...
function handleSelectConversation(payload) {
  ...
  const id = conversationChannelId(conversation);
  ...
  if (!isDesktop.value) {
    uni.navigateTo({
      url: `/pages/chat/detail?id=${id}&channelType=${channelType}`
    });
  }
}
...
function resolveAgentForMember(member) {
  if (!member || member.id === 'me') return null;
...
function handleSendMessage({ type, content, fileName, fileSize, replyRef, previewContent, fileType, url }) {
  ...
  if (type === 'file') {
    ...
    newMsg.previewContent = previewContent || '';
  ...
  if (conv) {
    ...
    else if (type === 'file') conv.lastMessage = `[文件] ${fileName}`;
    else if (type === 'voice') conv.lastMessage = '[语音]';
...
function openMemberContextMenu({ event, member }) {
  ...
  const canMention = activeConversation.value?.type === 'group' && normalizedMember?.id !== 'me';
...
function normalizeMember(member) {
  ...
  if (member.id === 'me') {
    return {
      id: 'me',
      nickname: appStore.currentUser?.nickname || '我',
      avatar: appStore.currentUser?.avatar || member.avatar || '',
...
function buildMemberContact(member) {
  ...
  if (member.id === 'me') {
    ...
    return {
      id: 'me',
      nickname: currentUser.nickname || member.nickname || '我',
...
function resolveName(uid) {
  if (uid === 'me') return appStore.currentUser?.nickname || '我';
  const conv = activeConversation.value;
  if (conv?.type === 'group') {

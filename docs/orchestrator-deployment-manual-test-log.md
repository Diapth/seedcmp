...
- Thread ID: thread_mq1afgliuch3zzzg
- Coordination ID:
- Deployment request ID: deploy_9ca1e2f9-d877-4131-9a12-307875b6ced4 for final AgentHub screenshot; deploy_b30143df-7a4b-4fb2-a11f-d99bbf894f61 for the final default AgentHub smoke rerun; deploy_e6eff95c-da30-4590-8dde-8435b8d14d17 and follow-up QA-page deployments from smoke hardening reruns
...
- Screenshot paths: docs/manual-test-artifacts/stage7/stage7-final-agenthub-preview.png; docs/manual-test-artifacts/stage7/stage7-final-agenthub-card-preview.png; docs/manual-test-artifacts/stage7/stage7-final-summary-preview.png
- Result: passed
- Failure reason:
- Notes: After the Stage 7 delivery-material commit and required stack restart, the final opt-in smoke initially exposed another browser-order edge case: the smoke selected `.deployment-card.last()` while the message list contained a fresh needs-target card above an older completed card. The smoke was patched to locate a non-terminal/actionable card by status/content, prefer the matching target candidate when the card offers one, wait for the target input to settle, retry the target commit if the first browser event does not stick, fall back to a normal Web chat field-update message, and re-resolve the confirmable card after the backend field update. A completion audit then found that the earlier accepted evidence proved the deployment card with `qa-test-page.html` but not the requested AgentHub coffee artifact. The final targeted command was rerun with `RUN_V3_CLOWDER_SMOKE=1 TEST_AGENT_A=xtz TEST_AGENT_B=dd TEST_DEPLOYMENT_TARGET=packages/api/data/agenthub-coffee-event/index.html corepack pnpm test:e2e -- tests-e2e/smoke-v3-orchestrator-deployment.spec.ts` and passed; after changing the smoke default target to the AgentHub artifact, `RUN_V3_CLOWDER_SMOKE=1 TEST_AGENT_A=xtz TEST_AGENT_B=dd corepack pnpm test:e2e -- tests-e2e/smoke-v3-orchestrator-deployment.spec.ts` also passed and created `deploy_b30143df-7a4b-4fb2-a11f-d99bbf894f61`. Final Web evidence used only the 3000 UI: the AgentHub deployment card target was `packages/api/data/agenthub-coffee-event/index.html`, the preview iframe opened in the right dock, and the rendered page showed the AgentHub coffee activity UI. Direct API/source checks for `deploy_9ca1e2f9-d877-4131-9a12-307875b6ced4` and `deploy_b30143df-7a4b-4fb2-a11f-d99bbf894f61` returned preview HTML with `<title>AgentHub 咖啡店活动页</title>` and a source package containing `index.html`.
